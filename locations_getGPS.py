...
df = pd.read_csv('./output_files/locations_unique_cleaned.txt',
                 header=None, names=['Collection_point'])

# Fetch GPS for the collection points in the dataframe
for index, row in df.iterrows():
    try:
        information = geocode(
            row['Collection_point'], provider='nominatim', 
            user_agent='xyz', timeout=5)
        df.loc[index, 'Longitude'] = information.geometry.loc[0].x
        df.loc[index, 'Latitude'] = information.geometry.loc[0].y

    except Exception as e:
        print(f'Geocoding failed for {row['Collection_point']}: 
              {e}')
...
gdf_joined = gpd.sjoin(gdf_points, gdf_boundaries,
                       how='left', predicate='within')

# Create a final DataFrame with the desired columns
final_df = gdf_joined[['Collection_point', 'Longitude',
                       'Latitude', 'NAZ_LAU1', 'NAZ_CZNUTS3']].copy()
final_df.rename(columns={'NAZ_LAU1': 'District_name',
                'NAZ_CZNUTS3': 'Region_name'}, inplace=True)
